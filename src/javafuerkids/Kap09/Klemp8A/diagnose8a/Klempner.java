  ...
  implements ActionListener, AdjustmentListener
{
  ...
  private JMenuItem[] Eintrag = new JMenuItem[8];
  ...
  public MouseListener Mauswaechter = new MouseAdapter()
  {
    public void mouseReleased (MouseEvent Ereignis)
    {
      if (Ereignis.isPopupTrigger())
        Popup.show (Platte, Ereignis.getX(),Ereignis.getY());
    } 
    ...
    {
      if (Ereignis.getButton() == MouseEvent.BUTTON1)
          Menu.show();
      if (Ereignis.getButton() == MouseEvent.BUTTON2)
          Popup.show (Platte, Ereignis.getX(),Ereignis.getY());
    } */
  }; 
  ...
  public void createMenu ()
  {
    ...
    Eintrag[4] = new JMenuItem ("Öffnen");
    Eintrag[5] = new JMenuItem ("Speichern");
    Eintrag[6] = new JMenuItem ("Drucken");
    Eintrag[7] = new JMenuItem ("Ende");
    setJMenuBar (Menuleiste);
    Menuleiste.add (Menu);
    for (int i = 0; i < 4; i++)
    {
      Menu.add (Eintrag[i]);
      Popup.add (Eintrag[i+4]);
      Eintrag[i].addActionListener(this);
      Eintrag[i+4].addActionListener(this);
   }
  ...
  public void actionPerformed (ActionEvent Ereignis)
  {
    ...
    // Öffnen
    if ((Quelle == Eintrag[0]) || (Quelle == Eintrag[4]))
    {
      try
      {
      ...
      {
        JOptionPane.showMessageDialog (null, "Problem beim Öffnen!");
        Sicher = false;
      }
    }
    //  Speichern
    if ((Quelle == Eintrag[1]) || (Quelle == Eintrag[5]))
    {
      try
   	  {
      ...
      {
        JOptionPane.showMessageDialog (null, "Problem beim Speichern!");
        Sicher = false;
      }
    }
    // Drucken
    if ((Quelle == Eintrag[2]) || (Quelle == Eintrag[6]))
    {
      try
      {
        int Wahl = DateiSucher.showOpenDialog (this);
      ...
      {
        JOptionPane.showMessageDialog (null, "Problem beim Drucken!");
      }
    }
    // Ende
    if ((Quelle == Eintrag[3]) || (Quelle == Eintrag[7]))
    {
      if (!Sicher)
      {
        int Wahl = JOptionPane.showConfirmDialog
